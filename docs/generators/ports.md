...
```bash
gohexa -generate port -generate port -feature <FeatureName> -output <OutputDirectory> -project <ProjectName>
```

### Example Commands
1. Generate Ports File:
```bash
gohexa -generate port -generate port -feature="Order" -output ./internal/core/ports -project my_project
```
